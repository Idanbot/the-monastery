...
export const ImportPreviewDialog: React.FC = () => {
  ...
  const importedTasks = Array.isArray(importPreview.imported) ? importPreview.imported : [];
  const newTasks = Array.isArray(importPreview.newTasks) ? importPreview.newTasks : [];
  const updatedTasks = Array.isArray(importPreview.updatedTasks) ? importPreview.updatedTasks : [];
  const unchangedTasks = Array.isArray(importPreview.unchangedTasks) ? importPreview.unchangedTasks : [];
      ...
      >
        <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800">
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Matching task IDs will be updated, new IDs will be added, and all other current tasks stay in
            place.
          </p>
        </div>
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-3 gap-2">
            <div className="rounded-lg bg-slate-50 dark:bg-slate-800 p-3 text-center border border-slate-200 dark:border-slate-700">
              <div className="text-2xl font-mono font-bold text-emerald-600">{newTasks.length}</div>
              <div className="text-xs text-slate-500">Backlog</div>
            </div>
            <div className="rounded-lg bg-slate-50 dark:bg-slate-800 p-3 text-center border border-slate-200 dark:border-slate-700">
              <div className="text-2xl font-mono font-bold text-indigo-600">{updatedTasks.length}</div>
              <div className="text-xs text-slate-500">Changed</div>
            </div>
            <div className="rounded-lg bg-slate-50 dark:bg-slate-800 p-3 text-center border border-slate-200 dark:border-slate-700">
              <div className="text-2xl font-mono font-bold text-slate-500">{unchangedTasks.length}</div>
          ...
          <div className="max-h-52 overflow-y-auto custom-scrollbar rounded-lg border border-slate-200 dark:border-slate-800 divide-y divide-slate-100 dark:divide-slate-800">
            {importedTasks.slice(0, 8).map((task: any) => (
            ...
            {importedTasks.length > 8 && (
              <div className="px-3 py-2 text-xs text-slate-400">+{importedTasks.length - 8} more</div>
            )}
          </div>
        </div>
